...
import { Users, Clapperboard, Crown, Gift, Heart, MessageSquare, Share2, Gem, Star, LayoutGrid, Video, ArrowLeft, Rss, UserPlus, Send, Circle } from 'lucide-react';
...
const videoPost = {
    ...
    title: "Aria's Adventure",
    viewers: '1.2k',
    videoUrl: 'https://cdn.pixabay.com/video/2024/04/10/206587-930038848_large.mp4',
    comments: [
        { user: 'Zane', text: 'Awesome stream! 🔥' },
        { user: 'Kael', text: 'What game is this?' },
        { user: 'Lyra', text: '❤️❤️❤️' },
        { user: 'Echo', text: 'Sent a Dragon! 🐲' },
    ]
};
                ...
                <div className="flex justify-between items-start">
                    <div className="flex items-center gap-3 bg-black/50 p-2 rounded-full pointer-events-auto">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-white/80">{post.viewers} Viewers</p>
                        </div>
                    </div>
                    <div className="bg-red-600 text-white px-3 py-1 text-sm font-semibold rounded-md pointer-events-auto">LIVE</div>
                </div>
            </div>
                 ...
                 <div className="flex items-end">
                    <div className="flex-grow space-y-2 max-h-48 overflow-y-auto pr-2 pointer-events-auto">
                       {post.comments.map((comment, index) => (
                         <p key={index} className="text-sm"><span className="font-bold text-yellow-300">{comment.user}:</span> {comment.text}</p>
                       ))}
                    </div>
                    <div className="flex flex-col gap-3 pl-3 pointer-events-auto">
                        <Button variant="ghost" size="icon" className="bg-white/10 rounded-full h-12 w-12 hover:bg-white/20"><Heart className="text-red-500 h-6 w-6" /></Button>
                        <Button variant="ghost" size="icon" className="bg-white/10 rounded-full h-12 w-12 hover:bg-white/20"><Gift className="text-yellow-400 h-6 w-6" /></Button>
...
export default function LivePage() {
    const topStreamers = [
        ...
        { name: 'Zane', viewers: 850, avatar: PlaceHolderImages.find(i => i.id === 'fighter-character')?.imageUrl },
    ];
    const topGifters = [
    ...
    <div className="min-h-screen bg-background/90 text-foreground">
        <div className="relative flex overflow-x-hidden bg-primary/10 border-b border-t border-primary/30 py-2 shadow-inner">
            <div className="animate-marquee whitespace-nowrap text-primary font-semibold">
                <span className="mx-4 tracking-wider">Aria just started a live stream! 🔴</span>
                <span className="mx-4 tracking-wider">Zane sent Kael a Dragon Gift! 🐲</span>
            </div>
        ...
        <div className="container mx-auto p-4 lg:p-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <aside className="lg:col-span-3 space-y-6">
                    ...
                    <Card className="bg-card/50 max-h-[calc(100vh-250px)]">
                        <CardHeader>
                            <CardTitle className="text-base">Streaming Now</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-3 overflow-y-auto">
                            {[...topStreamers, ...topStreamers, ...topStreamers].map((streamer, index) => (
                    ...
                    <Tabs defaultValue="live" className="w-full">
                        <div className="flex justify-between items-center mb-4">
                             <TabsList className="h-auto flex-wrap justify-center">
                                ...
                                <TabsTrigger value="friends" asChild><Link href="/socialive/friends"><UserPlus className="h-4 w-4 mr-1 sm:hidden"/>Friends</Link></TabsTrigger>
                                <TabsTrigger value="followers" asChild><Link href="/socialive/followers"><Rss className="h-4 w-4 mr-1 sm:hidden"/>Followers</Link></TabsTrigger>
                            </TabsList>
                        ...
                        <TabsContent value="live">
                             <div className="space-y-8 max-w-md mx-auto">
                                ...
                                <p className="text-center text-muted-foreground text-sm">Scroll up/down to switch streams (mock)</p>
                            </div>
                        </TabsContent>
                    </Tabs>
                </main>
